# credit: https://nitratine.net/blog/post/how-to-hash-passwords-in-python/
import hashlib
import os
...
key = hashlib.pbkdf2_hmac('sha256', (username + password).encode('utf-8'), salt, 100000)
...
new_key = hashlib.pbkdf2_hmac('sha256', (username + password).encode('utf-8'), salt, 100000)

# assert key != new_key # The keys are not the same thus the passwords were not the same
if key == new_key:# The keys are not the same thus the passwords were not the same
	print('Keys Equal!')
else:
